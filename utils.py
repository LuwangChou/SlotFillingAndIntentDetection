...
def getGloveEmbeddingWeight(embedding_path):
    embeddings_index = {}
    print('Indexing GLOVE word vectors.')
    with open(os.path.join(embedding_path,'glove.6B/glove.6B.100d.txt')) as f:
        for line in f:
...
MAX_NUM_WORDS = 20000
EMBEDDING_DIM = 100
def prepareEmbeddingsMatrix(embedding_path,in_vocab,embedding_size):
    print('Preparing embeddings matrix.')
    # prepare embedding matrix
    in_vocab_word = in_vocab['vocab']
    #print(in_vocab_word)
    #print(len(in_vocab_word))
    embeddings_index = getGloveEmbeddingWeight(embedding_path)
    num_words = min(MAX_NUM_WORDS,len(in_vocab_word) + 1)
    embedding_matrix = np.zeros((num_words,embedding_size))
    i = 0
    for word,i in in_vocab_word.items():
        ...
        if embedding_vector is not None:
            ...
            #else :
                #zeros_one_row = np.zeros((layer_size - EMBEDDING_DIM,1))

                #vector_one_row = embedding_vector[:EMBEDDING_DIM]

                #print(zeros_one_row[:,0].shape) #(28,)
                #print(vector_one_row.shape) #(100.)
                #print(len(np.concatenate((vector_one_row,zeros_one_row[:,0]),axis=0)))
                #print(embedding_matrix[i].shape)#(128,)
                #embedding_matrix[i] = np.concatenate((vector_one_row,zeros_one_row[:,0]),axis=0)
                
                
                
    mu = np.mean(embedding_matrix,axis=0)
